...
Walks  campaign_<TAG>/sweep_task*/  and:
  ...
  * emits a single SBI-ready index under  campaign_<TAG>/campaign_index/ :
        theta_all.npy   (N, 30)  inference-coordinate labels (the SBI prior space)
...
def main() -> int:
    ...
    task_dirs = sorted(d for d in root.glob("sweep_task*") if d.is_dir())
    if not task_dirs:
        sys.exit(f"no sweep_task* dirs under {root}")

...
assert len(PARAM_NAMES) == len(PARAM_UNITS) == NOMINAL_PARAMS.shape[0] == 36, \
    "registry must be 36-D (names/units/nominal); check HPC_single_run.py"
...
PARAM_BOUNDS = np.array([
    ...
    (1.15,    1.15),    # 30 gL         [nS]   FROZEN at 1.15 nS (GROUNDED Halliwell R_in=0.87 GOhm; Bucket-A)
...
assert PARAM_BOUNDS.shape == (36, 2)
...
''',
    ...
    for params, theta, iter_idx, seed_run in zip(
        ...
        # ── Across-cell / within-cell dispersion monitors ──────────────────
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument('--burst_design', default=None, choices=['A', 'B'],
                   help="Burst-validation design from burst_validation_box.py. "
                        "A: sweep only adaptation, freeze synapses burst-"
                        "favourably (decisive). B: joint burst-permissive "
                        "neuron+synapse sweep. OVERRIDES --sweep_group and "
                        "narrows the prior box. (Default: none.)")
...
def main():
    ...
    # ── Burst-validation design override (no-op unless --burst_design) ───────
    # Narrows PARAM_BOUNDS to a burst-favourable sub-box and moves the freeze
    # point of inactive axes off registry nominal. Applied BEFORE the rebuild
    # branch so the manifest records the SAME box the sweep actually drew from.
    # In-place mutation of the module globals keeps every downstream reference
    # (sample_theta, theta_to_natural) valid.
    _burst_active_idx = None
    if getattr(args, 'burst_design', None):
        import burst_validation_box as _bvb
        _ai, _pb, _nom = _bvb.apply_design(
            args.burst_design, PARAM_NAMES, PARAM_BOUNDS, NOMINAL_PARAMS)
        PARAM_BOUNDS[:, :]       = _pb
        NOMINAL_PARAMS[:]        = _nom
        PARAM_BOUNDS_THETA[:, :] = _compute_param_bounds_theta()
        _burst_active_idx        = _ai
        print(f"[main] burst_design={args.burst_design}: box narrowed; "
              f"sweeping {[PARAM_NAMES[i] for i in _ai]}")
    ...
    active_idx    = (_burst_active_idx if _burst_active_idx is not None
                     else resolve_sweep_group(args.sweep_group))
    inactive_idx  = sorted(set(range(N_DIMS)) - set(active_idx))

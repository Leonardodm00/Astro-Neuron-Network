...
CAMPAIGN_TAG="${CAMPAIGN_TAG:-cadex_hhgap_v1}"
# Burst-validation runs land in their OWN campaign dir so they never collide
# with the production SBI campaign. (BURST_DESIGN injected via qsub -v.)
...
[ -n "$BURST_DESIGN" ] && ARGS+=(--burst_design "$BURST_DESIGN")  # A|B narrowed box

python "$SCRIPT_PATH" "${ARGS[@]}"
EXIT_CODE=$?

echo "[post] rebuilding manifest for ${NODETAG}/task${IDX} ..."
python "$SCRIPT_PATH" --rebuild_manifest_only --out_dir "$OUTPUT_DIR" \
    ${BURST_DESIGN:+--burst_design "$BURST_DESIGN"} || true

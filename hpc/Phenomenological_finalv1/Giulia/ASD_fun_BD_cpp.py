...
def get_Neuronparam(**kwargs):
    ...
    # __ CAdEx (Gorski et al. 2021) -- phenomenological replacement of the HH model __
    ...
    'gl_ref': 10.0*nS,             # FIXED leak ref for the diffusion noise (= gL so sigma_V=sigma; Gorski RS gL)
                                    #   (decoupled from the swept per-neuron gl)

    # --- Swept CAdEx per-neuron axes (defaults; shadowed by run_args) ---
    'sigma':    4*mV,               # idx 0   noise amplitude (kept; not a Gorski param)
    'gbarA':  10.0*nS,             # idx 1   max subthreshold adaptation conductance g_bar_A (Gorski 2021 RS)
    'tauA':   200*ms,               # idx 4   adaptation time constant tau_A (Gorski 2021 RS)
    'DeltaT':   2*mV,               # idx 12  spike-initiation slope Delta_T (std AdEx)
    'VT':     -50.0*mV,            # idx 13  spike threshold V_T (Gorski 2021 RS)
    'delta_gA': 1.0*nS,            # idx 16  post-spike adaptation increment delta_g_A (Gorski 2021 RS)
    'gl':     10.0*nS,             # idx 30  leak conductance g_L (Gorski 2021 RS; R_in=100 MOhm)
    'VA':     -50.0*mV,            # idx 31  subthreshold adaptation activation V_A (Gorski 2021 RS)
    'DeltaA':   5*mV,               # idx 32  subthreshold adaptation slope Delta_A (> 0; Gorski 2021 RS)
...
def Neuronal_Network(Nn, Syn_pdist=None, ics=False, Simulated_network='Neuronal',
    ...
    ics : {False, 'rand'}
        ...
        (currently dormant -- kept for future use, see Report 3 _B.1).
    ...
    # __ CAdEx membrane dynamics (Gorski et al. 2021, eqs 2.1-2.2) __
    ...
    # SCALE I_inj x build-frozen unit draw bias_unit _ [-0.5, 0.5]), and the
    ...
    # 0 (it is assigned below). Using the namespace constant V0_span = VT_nominal - El
    ...
    # whole population crossed threshold together at t ~ 0 -> one synchronous spike per neuron,
    # then silence on the fast membrane. This was the source of the ~19 % one-spike-artifact
    # fraction observed in campaign rv3. Spreading V(0) uniformly across the subthreshold band
    # desynchronises the first wave; the onset transient shrinks to a short (<1 s) settling
    # period that is easily discarded in the feature extractor (warm-up mask: spk_t >= t_warmup).
    ...
    # an entire HPC sweep without anyone noticing (Report 3 _A.2).
    if connections is True:
        ...
        if len(Source) > 0:
            ...
            # Restore bouton positions if provided (Report 3 _A.3).
...
def _astrocyte_steady_state(params_astroGT, t_max=300.0):
    ...
    of Brian2's default zeros (Report 3 _A.1).
    ...
    def rhs(state, t):
        ...
        dGamma_A = 0.0
        # NOTE: I_exogenous is intentionally OMITTED here (notes _4). This routine
...
def astrocyte_connections(Astrocyte_group, Connection_dist,
    ...
    Two modes are supported, selected by `topology_mode`:
      ...
      * 'distance' (legacy, default for backward compatibility):
        ...
        `Connection_dist` (in um) is GJ-coupled bidirectionally.

      * 'wallach':
        Two astrocytes are GJ-coupled iff (i) they share a border in the
        JOINT Voronoi tessellation of neurons union astrocytes (i.e. they are
    ...
    Connection_dist : float
        Radius in um for the 'distance' mode (ignored in 'wallach' mode).
    topology_mode : {'distance', 'wallach'}
    Neuron_group : Brian2 NeuronGroup, optional
        Required for 'wallach' mode. Must expose `.x` and `.y`.
    gj_max_dist : float
        Soft cap in um applied on top of the Voronoi adjacency in 'wallach'
        mode (ignored in 'distance' mode). Default 150 um.
    ...
    """
    # Astrocyte coordinates as plain NumPy in um.
    ...
    if topology_mode == 'wallach':
        if Neuron_group is None:
            raise ValueError(
                ...
                "requires Neuron_group so that the joint (neurons union "
...
def Astrocyte_Group(N_astro, Simulated_network, seed_astro=None, ics='steady',
    ...
    ics : {'steady', 'rand', None}
        - 'steady' (default): integrate the isolated astrocyte ODEs to the
          ...
          (Report 3 _A.1). This is essential for HPC runs <~ 60 s where
    ...
    topology_mode : {'distance', 'wallach'}
        ...
        generate the GJC adjacency from cell positions:
          - 'distance' (legacy default): pairs within `conn_dist` um.
          - 'wallach': Delaunay-adjacency in the joint (neurons union astrocytes)
    ...
    gj_max_dist : float
        Soft cap in um for the Wallach branch (ignored in 'distance' mode).
    ...
    eqs_A = Equations('''
        ...
        # IP_3 dynamics (de Pitta form + exogenous tonic drive toward I_bias):
        ...
        # Exogenous tonic drive toward I_bias (de Pitta soft set-point, threshold-gated):
    ...
    Astro.O_N      = Params_astroGT['O_N']   # default; shadowed by run_args when swept

    # ----- Initial conditions (Report 3 _A.1) -----
    if ics == 'rand':
    ...
    #     neurons union astrocytes, capped at `gj_max_dist` um.
    #     Reference: Wallach et al. 2014, PLOS Comput. Biol. 10(12) e1003964;
    #     and De Pitta & Berry (eds.), Computational Glioscience, 2019, ch. 7.
    if connections is True:
...
def Gliotransmission(N_astro, Astro, ics='jitter', seed_astro=None):
    ...
    ics : {'jitter', 'rand', None}
        - 'jitter' (default): x_A initialised to 1 with +/-5% per-cell jitter
          (Report 3 _A.1, slight symmetry break); G_A = 0.
    ...
    if ics == 'jitter':
        ...
        # Legacy dormant branch -- fixed (was: 'synapses' undefined). Report 3 _B.2.
...
def Synapse_to_astro(synapse, Astro, connections,
    ...
    Two `topology_mode` rules are supported when `connections is True`:
      ...
      * 'distance' (legacy):
        ...
        Each synapse-astrocyte pair within `Conn_syn_astro_cutoff` um is
        sampled independently with p = exp(-d^2 / (2.sigma_A^2)). As a
      ...
      * 'wallach' (new default, Wallach et al. 2014-compatible):
        ...
        nearest one within `stoa_cutoff` um. Distance is measured from the
    ...
    stoa_cutoff : float
        Hard distance cutoff in um for the 'wallach' branch. Default 70 um.
    ...
    # Errors NOT swallowed (Report 3 _A.2).
    if connections is True:
        if topology_mode == 'wallach':
...
def Astro_to_Syn(Glio_release, synapse, connections):
    ...
    Astro_Syn = Synapses(Glio_release, synapse,
                         ...
                         ''',
                         method='rk4',                     # was: 'gsl ' (Report 3 _B.4)
...
# -----------------------------------------------------------------------------
# Wallach/De Pitta 2014-style topology helpers (numpy-only, importable from
...
def voronoi_astro_gj_pairs(N_pos, A_pos, gj_max_dist=150.0):
    ...
    of neurons + astrocytes (Wallach et al., 2014; De Pitta & Berry, 2019).

    Two astrocytes are GJC-coupled iff:
      1. their Voronoi cells share a border in the joint (neurons union astrocytes)
    ...
    (cf. Lallouette et al., 2014: d_max = 150 um).

    Parameters
    ----------
    N_pos : (Nn, 2) array
        Neuron (x, y) positions in um.
    A_pos : (Na, 2) array
        Astrocyte (x, y) positions in um.
    gj_max_dist : float
        Soft cap, in um. Set to numpy.inf to disable.
...
def nearest_astro_for_synapse(bouton_pos, A_pos, stoa_cutoff=70.0):
    ...
    bouton_pos : (n_syn, 2) array
        Bouton (x, y) positions in um.
    A_pos : (Na, 2) array
        Astrocyte (x, y) positions in um.
    stoa_cutoff : float
        Hard distance cutoff in um. Synapses whose nearest astrocyte lies

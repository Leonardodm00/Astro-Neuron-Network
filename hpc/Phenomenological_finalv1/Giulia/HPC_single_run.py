...
#     * neuron positions    (uniform random in [0, c_max]^2)
#     * synapse (i,j) pairs (Bernoulli with conn_prob, no autapses)
#     * synapse bouton positions (Gaussian around post-soma, sigma = bouton_sigma)
...
#   raster_astrocytes.png  astrocyte Ca^2_ event raster + event-rate trace
...
PARAM_NAMES = [
    ...
    'I_inj',                                     # 34    Neuron  (swept per-neuron bias SCALE)
    'Cm',                                        # 35    Neuron  (FROZEN at RS; tau_m=Cm/gL=20 ms)
    'O_N',                                       # 36    Astrocyte  (mGluR glutamate-binding rate; SWEPT sensitivity)
]
...
NOMINAL_PARAMS = np.array([
    ...
    2.0,        # 12 DeltaT         mV    (Delta_T, FROZEN Bucket-A quartet)
    -50.0,      # 13 VT             mV    (spike threshold, FROZEN; GROUNDED Gunhanlar -50.9; gap=7.2mV)
    0.3,        # 14 g_ampa         nS    (Doorn Table 1 scale)
    0.3,        # 15 g_nmda         nS    (Doorn Table 1 scale)
    1.0,      # 16 delta_gA       nS    (deltag_A; Doorn scale, mid-log of wide bounds)
    ...
    10.0,       # 30 gL             nS    (leak conductance g_L, FROZEN; GROUNDED Halliwell R_in=0.87 GOhm)
...
NEURON_PARAMS  = ['Sigma', 'gbarA', 'delta_gA', 'tauA',
                  'VA', 'DeltaA', 'VR', 'I_inj', 'Cm']        # 9 free CAdEx axes (Cm carries tau_m)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # __ Paths ________________________________________________________________
    ...
    # __ Simulation ___________________________________________________________
    ...
    # __ Population sizes _____________________________________________________
    p.add_argument('--Nn', type=int, default=100,
                   help='Number of neurons (default: %(default)s). Overridden if '
                        '--density is given.')
    p.add_argument('--Na', type=int, default=43,
                   help='Number of astrocytes (default: %(default)s).  '
                        'Ignored when --mode=Neuronal. Overridden/scaled if '
                        '--density is given.')
    p.add_argument('--density', type=float, default=None, metavar='PER_MM2',
                   help='Areal NEURON density [neurons/mm^2]. If set, Nn is '
                        'DERIVED from the arena: Nn = round(density*(c_max/1000)^2), '
                        'so density is held fixed as c_max changes (the scale-'
                        'invariance prerequisite). Overrides --Nn.')
    p.add_argument('--density_astro', type=float, default=None, metavar='PER_MM2',
                   help='Areal ASTROCYTE density [astrocytes/mm^2] (Full mode). If '
                        'omitted while --density is set, Na is scaled to preserve '
                        'the --Na:--Nn ratio (astrocyte density also held fixed).')

    # __ Arena ________________________________________________________________
    p.add_argument('--c_max', type=float, default=1100.0, metavar='UM',
                   help='Side length of the square arena in um (default: %(default)s).')

    # __ Synapse connectivity __________________________________________________
    p.add_argument('--conn_prob', type=float, default=0.107, metavar='PROB',
                   help='Bernoulli connection probability for neuron->neuron '
                        ...
                        'Used only when --conn_rule flat.')
    p.add_argument('--conn_rule', default='flat', choices=['flat', 'weibull'],
                   help='Neuron->neuron wiring rule. "flat" (default) uses the '
                        'distance-independent Bernoulli --conn_prob. "weibull" '
                        'uses the distance kernel p(d)=p0*exp(-(d/d0)**beta).')
    p.add_argument('--p0_conn', type=float, default=0.6, metavar='PROB',
                   help='[conn_rule=weibull] connection probability at d=0 '
                        '(default: %(default)s).')
    p.add_argument('--d0_conn', type=float, default=100.0, metavar='UM',
                   help='[conn_rule=weibull] kernel characteristic length [um] '
                        '(default: %(default)s).')
    p.add_argument('--beta_conn', type=float, default=1.0, metavar='EXP',
                   help='[conn_rule=weibull] kernel stretch exponent '
                        '(1 = plain exponential; <1 heavier tail; '
                        'default: %(default)s).')
    p.add_argument('--conn_periodic', action='store_true',
                   help='[conn_rule=weibull] use minimum-image (flat-torus) '
                        'distances instead of bounded Euclidean.')
    ...
    p.add_argument('--displ_bias', type=float, default=15.0, metavar='UM',
                   help='Constant um offset added to every sampled distance '
                        'from the post-soma so the bouton never coincides '
                        'with the soma centre (default: %(default)s).')

    # __ Astrocyte connectivity ________________________________________________
    p.add_argument('--topology_mode', default='wallach',
                   ...
                   help='Rule for building astrocyte connectivity from cell '
                        ...
                        'uses the joint (neurons union astrocytes) Voronoi '
    ...
    # __ Seeds ________________________________________________________________
    ...
    # __ Swept parameters _____________________________________________________
    ...
    g.add_argument('--g_na',       type=float, default=None, metavar='COEFF',
                   help='g_na coefficient  (SI value = coeff x mS cm_^2 x area)')
    g.add_argument('--g_kd',       type=float, default=None, metavar='COEFF',
                   help='g_kd coefficient  (SI value = coeff x mS cm_^2 x area)')
    # NEW axes (idx 14-29)
    ...
    # __ Figure options ________________________________________________________
...
# =============================================================================
# Fixed-density population sizing
# =============================================================================

def resolve_population_from_density(Nn, Na, c_max,
                                    density=None, density_astro=None,
                                    verbose=True):
    """Optionally derive population counts from fixed areal densities.

    Holds density rho = N / area fixed. For a square culture of side c_max [um],
    the area is (c_max / 1000)**2 mm^2, so N = round(density * area). This is the
    structural prerequisite for size-invariance: only when rho is held constant
    does a kernel's per-neuron in-degree distribution become independent of c_max
    (see the in-degree convergence study), so the same prior transfers across
    culture sizes.

    Parameters
    ----------
    Nn, Na : int
        Incoming neuron / astrocyte counts (the --Nn/--Na CLI values).
    c_max : float [um]
        Arena side length.
    density : float or None [neurons/mm^2]
        If None, Nn is returned unchanged (legacy behaviour). If given, Nn is
        derived from the arena and OVERRIDES the incoming Nn.
    density_astro : float or None [astrocytes/mm^2]
        If given (and density is given), Na is derived independently. If omitted
        while `density` is set, Na is scaled to PRESERVE the incoming Na:Nn ratio,
        so astrocyte density is held fixed too and a Full-mode network stays
        self-consistent across sizes.

    Returns
    -------
    (Nn_out, Na_out) : tuple[int, int]
    """
    area_mm2 = (c_max / 1000.0) ** 2
    Nn_out, Na_out = int(Nn), int(Na)
    if density is not None and density > 0:
        Nn_out = max(1, int(round(density * area_mm2)))
        if density_astro is not None and density_astro > 0:
            Na_out = max(0, int(round(density_astro * area_mm2)))
            astro_note = f"rho_astro={density_astro}/mm^2"
        elif Nn > 0:
            Na_out = max(0, int(round(Nn_out * (Na / Nn))))   # preserve Na:Nn ratio
            astro_note = f"Na:Nn ratio {Na}/{Nn} preserved"
        else:
            astro_note = "Na unchanged (incoming Nn==0)"
        if verbose:
            print(f"[density] rho_neuron={density}/mm^2  c_max={c_max}um  "
                  f"area={area_mm2:.4f}mm^2  ->  Nn={Nn_out}, Na={Na_out}  ({astro_note})")
    return Nn_out, Na_out
...
def build_topology(Nn, Na, c_max,
                   ...
                   gj_max_dist=150.0,
                   conn_rule='flat',          # 'flat' (legacy default) | 'weibull'
                   p0_conn=0.6,               # [weibull] kernel p(0)
                   d0_conn=100.0,             # [weibull] kernel length scale [um]
                   beta_conn=1.0,             # [weibull] kernel stretch exponent
                   conn_periodic=False) -> dict:   # [weibull] minimum-image torus
    """
    Build the full network topology using plain NumPy / SciPy.
    No Brian2 device is touched.

    Bouton placement follows the morphometric chi^2(4) Sholl distribution
    ...
    `displ_bias` um) and the bouton is placed along the line connecting
    ...
    by `topology_mode`:
      ...
      * 'wallach' (default, Wallach et al. 2014):
        GJC: Delaunay-adjacency in the JOINT (neurons union astrocytes) Voronoi
             tessellation, capped at `gj_max_dist` um.
        StoA: each bouton is assigned to its NEAREST astrocyte within
              `stoa_cutoff` um -- strictly 1 astrocyte per synapse.
        The `gj_dist` and `stoa_sigma` parameters are ignored.

      * 'distance' (legacy):
        GJC: every astrocyte pair within `gj_dist` um is connected (KDTree
             radius query).
        StoA: every (synapse, astrocyte) pair within `stoa_cutoff` um is
              independently accepted with probability
              p = exp(-d^2 / (2.stoa_sigma^2)). A synapse may be linked to
    ...
    -------
    dict with keys
        N_pos       (Nn, 2) float64  -- neuron (x, y) positions in um
        S_i, S_j    (n_syn,) int32   -- synapse pre/post indices
        S_x_syn     (n_syn,) float64 -- bouton x position in um
        S_y_syn     (n_syn,) float64 -- bouton y position in um
        A_pos       (Na, 2) float64  -- astrocyte (x, y) positions in um
    ...
    # __ Neuron positions ______________________________________________________
    N_pos = rng_n.uniform(0.0, c_max, (Nn, 2))

    # __ Synapse connectivity (neuron -> neuron) _______________________________
    # Two selectable rules. 'flat' is the legacy distance-independent Bernoulli
    # and is preserved verbatim, so a flat run is byte-identical to before. The
    # kernel params (p0_conn, d0_conn, beta_conn, conn_periodic) are IGNORED
    # under 'flat'. See ASD_fun_BD_cpp.weibull_connections for the kernel rule.
    if conn_rule == 'flat':
        # Vectorised: draw a full NnxNn matrix, force the diagonal to fail.
    ...
    elif conn_rule == 'weibull':
        # Distance-dependent stretched-exponential kernel
        #   p(d) = p0_conn * exp(-(d/d0_conn)**beta_conn).
        # The helper reseeds from seed_synapse internally, keeping the
        # synapse-wiring randomness on the same seed as the flat rule. Note this
        # leaves rng_s unconsumed (the flat path consumes it via .random), so
        # the downstream bouton draws (which also use rng_s) start from a fresh
        # state under weibull; both rules remain fully deterministic given seeds.
        from ASD_fun_BD_cpp import weibull_connections
        S_i, S_j = weibull_connections(
            N_pos, p0=p0_conn, d0=d0_conn, beta=beta_conn,
            c_max=c_max, seed=seed_synapse, periodic=conn_periodic,
        )
    else:
        raise ValueError(
            f"conn_rule must be 'flat' or 'weibull', got {conn_rule!r}"
        )
    n_syn = len(S_i)

    # __ Synapse bouton positions (chi^2(4) Sholl PDF from CSV) _________________
    ...
    # __ Astrocyte positions ___________________________________________________
    ...
    # __ Gap-junction connectivity _____________________________________________
    if topology_mode == 'wallach':
        # Joint Voronoi (_ Delaunay) over neurons union astrocytes; keep only
        # astrocyte-astrocyte edges; soft cap at `gj_max_dist` um.
        # Wallach et al. 2014, Fig. 2; De Pitta & Berry 2019, ch. 7, eq. (7.9).
        from ASD_fun_BD_cpp import voronoi_astro_gj_pairs
        GJ_i, GJ_j = voronoi_astro_gj_pairs(N_pos, A_pos,
                                            gj_max_dist=gj_max_dist)
    else:
        # Legacy KDTree radius rule -- every pair within `gj_dist` um.
        tree_a = KDTree(A_pos)
    ...
    # __ Synapse -> Astrocyte connectivity ______________________________________
...
def build_and_run(topo: dict, args, params: np.ndarray) -> dict:
    ...
    """
    # __ Brian2 device setup (must precede start_scope and group creation) ____
    from brian2 import (
    ...
    # __ Library import ________________________________________________________
    ...
    # __ Binomial_fun (BINV: inverse-transform sampling) ________________________
    ...
    #
    # This REPLACES the former Bernoulli-sum loop  k = sum_{i=1..n} 1[rand() < p].
    # Both are exact samplers of Binomial(n, p); validated statistically
    # equivalent on the cpp_standalone backend (KS p >= 0.5 across the nominal
    # (n, p) grid, and ensemble-equivalent C_async(T) / int Y_S dt in the
    # async-release network). BINV consumes exactly ONE rand() per call instead
    # of up to n, which is the source of the speed-up.
    #
    # q^n is built with a short loop (n = floor(x_S/x0) <= floor(1/x0), small)
    # so no <cmath>/pow dependency is injected into the generated code. The cpp
    # implementation returns double (matches return_unit=1 and the /dt in the
    # `nar` synapse equation); the former returned int.
...
''',
    ...
    # __ Synapse positions as (N, 2) array in um _______________________________
    syn_positions = np.column_stack([topo['S_x_syn'], topo['S_y_syn']])

    # __ Build neuronal network (explicit (i,j) lists from pass 1) _____________
    ...
    # Override neuron positions with pass-1 values (um -> metre).
    ...
    if args.mode == 'Full':
        # __ Astrocyte group ___________________________________________________
    ...
    # __ Compile _______________________________________________________________
    ...
    # __ Unpack parameter vector _______________________________________________
    run_args = {
    ...
    # __ Execute _______________________________________________________________
    ...
    # __ Harvest spikes ________________________________________________________
...
def plot_neuronal_raster(spk_N_t, spk_N_i, simtime_s, Nn, params,
    ...
    title_p = (f'sigma={params[0]:.2f} mV   g_AHP={params[1]:.1f} nS   '
               f'g_na={params[12]:.1f}   g_kd={params[13]:.1f}   '
               f'alpha_syn={params[11]:.2f}')
...
def plot_astrocyte_raster(spk_A_t, spk_A_i, simtime_s, Na, params,
    ...
    if len(spk_A_t):
        ...
        ax.text(0.5, 0.5, 'No astrocyte Ca^2_ events detected',
                ha='center', va='center', transform=ax.transAxes,
                fontsize=12, color='gray')
    ...
    title_p = (f'Tau_Ca={params[4]:.1f} s   '
               ...
               f'alpha_syn={params[11]:.2f}')
    ax.set_title(f'Astrocyte Ca^2_ events -- {Na} cells, {simtime_s:.0f} s\n{title_p}',
...
def _draw_voronoi_overlay(ax, N_pos, A_pos, c_max,
    ...
    clipped to the [0, c_max]^2 arena. Astrocyte Voronoi cells are lightly
    tinted to make the astrocyte "anatomical domains" visible at a glance,
    in the spirit of Wallach et al. 2014, Fig. 2B/C.

    Cell positions must be supplied as (N, 2) numpy arrays in um.
    """
    ...
    # Clip polygons to the arena rectangle [0, c_max]^2 (Sutherland-Hodgman).
    arena = np.array([
...
def plot_spatial_layout(topo: dict, c_max: float, mode: str,
    ...
    # __ Voronoi tessellation (neurons union astrocytes) _________________________
    ...
    # __ Arena boundary _______________________________________________________
    ax.add_patch(Rectangle((0.0, 0.0), c_max, c_max,
                           fill=False, edgecolor='#666666',
                           linewidth=1.0, linestyle='--', alpha=0.7, zorder=1))

    # __ Boutons (small red dots) _____________________________________________
    n_syn = len(topo['S_i'])
    if n_syn:
    ...
    # __ Somata _______________________________________________________________
    if mode == 'Full' and len(topo['A_pos']):
    ...
    # __ Frame & cosmetics ____________________________________________________
    pad = 0.04 * c_max
...
def plot_spatial_connectivity(topo: dict, c_max: float, mode: str,
    ...
    # __ StoA links (bouton -> astrocyte, drawn first, lightest) ______________
    ...
    # __ GJ links (deduplicate bidirectional) ________________________________
    n_gj_unique = 0
    ...
    # __ Desaturated somata + boutons (background context) ___________________
    n_syn = len(topo['S_i'])
    if n_syn:
        ax.scatter(topo['S_x_syn'], topo['S_y_syn'],
    ...
    # __ Legend (proxy artists for the links) ________________________________
...
def plot_3d_layered_topology(topo: dict, c_max: float, mode: str,
    ...
    # __ Voronoi tessellation, projected onto each plane _____________________
    if mode == 'Full' and len(topo['A_pos']):
    ...
    # __ Neuron <-> neuron synapses (very faint, green dashed) ________________
    if len(topo['S_i']):
    ...
    # __ Bouton -> astrocyte links (purple, crossing planes) _________________
    if mode == 'Full' and len(topo.get('StoA_i', [])):
        for si, aj in zip(topo['StoA_i'], topo['StoA_j']):
            ax.plot([topo['S_x_syn'][si], topo['A_pos'][aj, 0]],
                    [topo['S_y_syn'][si], topo['A_pos'][aj, 1]],
    ...
    # __ Astrocyte <-> astrocyte GJC (amber, thick, in the astro plane) _______
    n_gj_unique = 0
    if mode == 'Full' and len(topo.get('GJ_i', [])):
        seen = set()
        for i, j in zip(topo['GJ_i'], topo['GJ_j']):
    ...
    # __ Boutons in the neuron plane (red dots) _____________________________
    if len(topo['S_i']):
        ax.scatter(topo['S_x_syn'], topo['S_y_syn'],
    ...
    # __ Neurons & astrocytes _______________________________________________
    ax.scatter(topo['N_pos'][:, 0], topo['N_pos'][:, 1],
    ...
    # __ Legend (proxy artists with link colours) ___________________________
    from matplotlib.lines import Line2D
    proxies = [
        Line2D([], [], color=_VIZ_NEURON_FACE, marker='o', linewidth=0,
               markeredgecolor=_VIZ_NEURON_EDGE,
    ...
    # __ Axes _______________________________________________________________
    pad = 0.04 * c_max
    ax.set_xlim(-pad, c_max + pad)
    ax.set_ylim(-pad, c_max + pad)
    ax.set_zlim(-0.1 * c_max, Z_ASTRO + 0.1 * c_max)

    ax.set_xlabel('x (um)', fontsize=10, labelpad=8)
    ax.set_ylabel('y (um)', fontsize=10, labelpad=8)
...
def _draw_voronoi_3d_at_z(ax, N_pos, A_pos, c_max, z, which):
    """
    Project the joint (neurons union astrocytes) Voronoi tessellation onto a
...
def write_summary(path, args, params, topo, results):
    ...
    lines = [
        ...
        + (f'target = {args.conn_prob:.4f})' if args.conn_rule == 'flat'
           else 'rule = weibull)'),
        f'    Gap junctions     : {n_gj}',
        f'    StoA links        : {n_stoa}',
        '',
        '  Connectivity geometry',
        f'    Arena             : [{args.c_max:.0f} um]^2',
        f'    conn_rule         : {args.conn_rule}',
        (f'    conn_prob         : {args.conn_prob}  (flat Bernoulli)'
         if args.conn_rule == 'flat' else
         f'    weibull kernel    : p0={args.p0_conn}  d0={args.d0_conn} um  '
         f'beta={args.beta_conn}  periodic={args.conn_periodic}'),
        f'    bouton placement  : chi^2(4) Sholl PDF from synapse_pdist.csv',
        f'    displ_bias        : {args.displ_bias} um',
        f'    gj_dist           : {args.gj_dist} um',
        f'    stoa_cutoff       : {args.stoa_cutoff} um',
        f'    stoa_sigma        : {args.stoa_sigma} um',
        '',
        '  Simulation',
...
def main():
    ...
    # __ Fixed-density population sizing (no-op unless --density is given) _____
    ...
    # __ Load Syn_pdist CSV (required for chi^2(4) Sholl bouton placement) _____
    syn_prob_csv = _resolve_syn_pdist_csv(args)

    # __ Pass 1: topology _____________________________________________________
    t0_topo = _wall.time()
    topo = build_topology(
        ...
        p0_conn=args.p0_conn,
        d0_conn=args.d0_conn,
        beta_conn=args.beta_conn,
        conn_periodic=args.conn_periodic,
    )
    t_topo = _wall.time() - t0_topo
    save_topology(topo, args.out_dir)

    # __ Spatial layout plots (independent of pass 2 -- done up-front so the
    #    user has a visual sanity check even if the simulation later fails) __
    plot_spatial_layout(
    ...
    # __ Pass 2: build, compile, run ___________________________________________
    results = build_and_run(topo, args, params)
    results['t_topo_s'] = t_topo

    # __ Save spike data _______________________________________________________
    ...
    # __ Plots _________________________________________________________________
    ...
    # __ Summary _______________________________________________________________
    write_summary(

...
if 'I_inj' not in PARAM_NAMES:                       # stale 34-D registry -> repair
    ...
    NOMINAL_PARAMS[13] = -48.0     # VT      frozen (repair stale -50)
    NOMINAL_PARAMS[30] = 0.9       # gL      frozen, Doorn scale (repair stale 10)
    NOMINAL_PARAMS = np.append(NOMINAL_PARAMS, 4.0)  # idx 34 I_inj nominal (~mid of [1,7.5])
assert len(PARAM_NAMES) == len(PARAM_UNITS) == NOMINAL_PARAMS.shape[0] == 35, \
    "registry must be 35-D (names/units/nominal); check HPC_single_run.py"
...
PARAM_BOUNDS = np.array([
    ...
    (0.009,   3.6),     # 1  gbarA      [nS]   max subthreshold adaptation g_barA      (log)  [Doorn: x g_L ratio 0.090]
    ...
    (-48.0, -48.0),     # 13 VT         [mV]   FROZEN at -48 mV (spike threshold; Bucket-A quartet; gap=5mV)
    (0.05,    1.0),     # 14 g_ampa     [nS]   Doorn Table 1 empirical range [0.05, 1]
    (0.01,    1.0),     # 15 g_nmda     [nS]   Doorn Table 1 [0, 1]; floor 0.01 to keep log axis valid
    (0.00075, 0.9),     # 16 delta_gA   [nS]   post-spike adaptation increment dg_A    (log)  [Doorn: wide, x0.015..x0.090]
    ...
    (0.9,     0.9),     # 30 gL         [nS]   FROZEN at 0.9 nS (Doorn: 0.3 mS/cm^2 * 300 um^2; Bucket-A)
    ...
    (1.0,     7.5),     # 34 I_inj      [pA]   per-neuron bias SCALE (Doorn scale: dV=I_inj*u/g_L; linear)
], dtype=np.float64)

N_DIMS = PARAM_BOUNDS.shape[0]
assert PARAM_BOUNDS.shape == (35, 2)
...
KERNEL_BOUNDS = np.array([
    (0.05, 1.0),     # p0_conn   [dimensionless]  p(0); scales mean in-degree
    (20.0, 40.0),    # d0_conn   [um]             SHORT range (convergence by 1 mm^2)
    (0.8,  1.0),     # beta_conn [dimensionless]  light tail (1 == plain exponential)
], dtype=np.float64)

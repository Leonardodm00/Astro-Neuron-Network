...
PARAM_NAMES = [
    ...
    'Cm',                                        # 35    Neuron  (SWEPT membrane capacitance; tau_m axis, gl frozen)
]
...
NOMINAL_PARAMS = np.array([
    ...
    -51.0,      # 13 VT             mV    (spike threshold, FROZEN; GROUNDED Gunhanlar -50.9; gap=7.2mV)
    ...
    1.15,       # 30 gL             nS    (leak conductance g_L, FROZEN; GROUNDED Halliwell R_in=0.87 GOhm)
    -48.0,      # 31 VA             mV    (subthreshold adaptation activation; shifted -3.0 to track VT -48->-51)
    5.0,        # 32 DeltaA         mV    (Δ_A, subthreshold adaptation slope, > 0)
    -58.2,      # 33 VR             mV    (reset potential; shifted -3.2 to track El -55->-58.2)
    4.0,        # 34 I_inj          pA    (per-neuron bias SCALE; mid of [1,7.5])
    17.25,      # 35 Cm             pF    (SWEPT; nominal tau_m=15 ms at gl=1.15 nS)
])
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument('--d0_conn', type=float, default=100.0, metavar='UM',
                   help='[conn_rule=weibull] kernel characteristic length [µm] '
                        '(default: %(default)s).')
...
def build_topology(Nn, Na, c_max,
    ...
    # ── Synapse connectivity (neuron -> neuron) ───────────────────────────────
...
def write_summary(path, args, params, topo, results):
    ...
    lines = [
        ...
        (f'    conn_prob         : {args.conn_prob}  (flat Bernoulli)'
         ...
         f'    weibull kernel    : p0={args.p0_conn}  d0={args.d0_conn} µm  '
...
def main():
    ...
    # ── Fixed-density population sizing (no-op unless --density is given) ─────

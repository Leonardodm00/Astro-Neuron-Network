...
| flag | default | meaning |
|---|---|---|
| `--workers` | `1` | parallel worker processes (spawn context; one per topology) |
...
walk_campaign(campaign_dir, out_dir, lib_path, cfg, workers=1,
    ...
    # Pool (one worker process per topology); prints a one-line summary and
    # returns the list of per-topology (topo_idx, n_found, n_done) results

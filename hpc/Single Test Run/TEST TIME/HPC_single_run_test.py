...
#   raster_astrocytes.png  astrocyte Ca2+ event raster + event-rate trace
...
NOMINAL_PARAMS = np.array([
    ...
    80.0,       # g_na  coeff    (= 1.6 x 50)
    6.5,        # g_kd  coeff    (= 1.3 x 5)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument('--topology_mode', default='wallach',
                   ...
                   help='Rule for building astrocyte connectivity from cell '
                        ...
                        'uses the joint (neurons U astrocytes) Voronoi '
    ...
    g.add_argument('--g_na',       type=float, default=None, metavar='COEFF',
                   help='g_na coefficient  (SI value = coeff x mS cm-2 x area)')
    g.add_argument('--g_kd',       type=float, default=None, metavar='COEFF',
                   help='g_kd coefficient  (SI value = coeff x mS cm-2 x area)')
    # NEW axes (idx 14-29)
...
def build_topology(Nn, Na, c_max,
    ...
    Bouton placement follows the morphometric chi2(4) Sholl distribution
    ...
    by `topology_mode`:
      ...
      * 'wallach' (default, Wallach et al. 2014):
        GJC: Delaunay-adjacency in the JOINT (neurons U astrocytes) Voronoi
      ...
      * 'distance' (legacy):
        ...
        StoA: every (synapse, astrocyte) pair within `stoa_cutoff` um is
              ...
              p = exp(-d2 / (2.stoa_sigma2)). A synapse may be linked to
    ...
    # -- Synapse bouton positions (chi2(4) Sholl PDF from CSV) -----------------
    ...
    if topology_mode == 'wallach':
        # Joint Voronoi (== Delaunay) over neurons U astrocytes; keep only
...
''',
    ...
    # -- Synapse positions as (N, 2) array in um -------------------------------
...
def plot_astrocyte_raster(spk_A_t, spk_A_i, simtime_s, Na, params,
    ...
    if len(spk_A_t):
        ...
        ax.text(0.5, 0.5, 'No astrocyte Ca2+ events detected',
                ha='center', va='center', transform=ax.transAxes,
                fontsize=12, color='gray')
    ...
    title_p = (f'Tau_Ca={params[4]:.1f} s   '
               ...
               f'alpha_syn={params[11]:.2f}')
    ax.set_title(f'Astrocyte Ca2+ events -- {Na} cells, {simtime_s:.0f} s\n{title_p}',
...
def plot_spatial_layout(topo: dict, c_max: float, mode: str,
    ...
    # -- Voronoi tessellation (neurons U astrocytes) -------------------------
...
def _draw_voronoi_3d_at_z(ax, N_pos, A_pos, c_max, z, which):
    """
    Project the joint (neurons U astrocytes) Voronoi tessellation onto a
...
def write_summary(path, args, params, topo, results):
    ...
    lines = [
        ...
        f'    bouton placement  : chi2(4) Sholl PDF from synapse_pdist.csv',
    ...
    lines += [
        ...
        f'    TOTAL script      : {results.get("t_script_total_s", 0.0):.2f} s '
        f'({results.get("t_script_total_s", 0.0) / 60.0:.2f} min)',
        '',
        '  Output directory',
        f'    {args.out_dir}',
        '',
...
def main():
    t_script_start = _wall.time()          # -- master wall-clock timer --
    ...
    # -- Load Syn_pdist CSV (required for chi2(4) Sholl bouton placement) -----
    ...
    # -- Summary ---------------------------------------------------------------
    t_script_total = _wall.time() - t_script_start
    results['t_script_total_s'] = t_script_total
    ...
    print(f'[done] Total wall-clock time: {t_script_total:.2f} s '
          f'({t_script_total / 60.0:.2f} min)')

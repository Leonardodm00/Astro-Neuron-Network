...
# flat    : distance-independent Bernoulli, conn_prob ~ U[LO,HI] (legacy default)
# weibull : distance kernel p(d)=p0*exp(-(d/d0)**beta); the sweep DRAWS
#           (p0,d0,beta) per topology from the kernel prior in HPC_main_sweep.py
#           (KERNEL_BOUNDS), so DO NOT set fixed p0/d0/beta here. Flip to
#           "weibull" to activate; leaving "flat" reproduces existing runs.
CONN_RULE="flat"            # flat | weibull
CONN_PERIODIC=0             # 1 => minimum-image (flat-torus) distances (weibull only)
# Optional kernel-prior overrides (leave empty to use KERNEL_BOUNDS defaults):
P0_LO=""; P0_HI=""
D0_LO=""; D0_HI=""
BETA_LO=""; BETA_HI=""
...
# --- Population sizes ---------------------------------------------------
NN=100                  # number of neurons (ignored if DENSITY is set)
NA=43                   # number of astrocytes (ignored when MODE=Neuronal)
DENSITY=""              # neurons/mm^2; if set, Nn=round(DENSITY*(C_MAX/1000)^2)
DENSITY_ASTRO=""        # astrocytes/mm^2; if empty while DENSITY set, Na:Nn kept
...
if [ -n "$DENSITY_ASTRO" ]; then
    ARGS+=(--density_astro "$DENSITY_ASTRO")
fi
if [ -n "$SEED_MASTER" ]; then
    ARGS+=(--seed_master "$SEED_MASTER")
fi

# Weibull-only options (no-ops under CONN_RULE=flat) ----------------------
[ "$CONN_PERIODIC" -eq 1 ] && ARGS+=(--conn_periodic)
[ -n "$P0_LO"   ] && ARGS+=(--p0_lo   "$P0_LO")
[ -n "$P0_HI"   ] && ARGS+=(--p0_hi   "$P0_HI")
[ -n "$D0_LO"   ] && ARGS+=(--d0_lo   "$D0_LO")
[ -n "$D0_HI"   ] && ARGS+=(--d0_hi   "$D0_HI")
[ -n "$BETA_LO" ] && ARGS+=(--beta_lo "$BETA_LO")
[ -n "$BETA_HI" ] && ARGS+=(--beta_hi "$BETA_HI")

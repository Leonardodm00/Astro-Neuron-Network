...
for q in "${QUEUES[@]}"; do
    ...
    if [ "$ntasks" -gt 1 ]; then
        jid=$(qsub -q "$name" \
            ...
            -J "0-$((ntasks-1))%${conc}" \
            -v "NODETAG=${name},SEED_BASE=${sb},SWEEP_GROUP=${SWEEP_GROUP},MODE=${MODE},CONN_RULE=${CONN_RULE},CONN_PERIODIC=${CONN_PERIODIC},DENSITY=${DENSITY}" \
            "$WORKER")
    else
...
echo "    python status_sweep.py campaign_300k_v1 --target ${TARGET}"
